#!/usr/bin/env bash
# CommitForge Installer — Debian / Ubuntu / Linux Mint
# Usage: curl -fsSL https://raw.githubusercontent.com/estevam5s/commitforge/main/cli-commit/install-debian.sh | bash
...
echo -e "${BOLD}"
echo "  ╔══════════════════════════════════╗"
echo "  ║  CommitForge Installer — Debian  ║"
echo "  ╚══════════════════════════════════╝"
echo -e "${NC}"

# Detect Debian-based system
command -v apt-get &>/dev/null || error "apt-get não encontrado. Este script é para Debian/Ubuntu/Mint."
...
sudo apt-get install -y -qq \
  ...
  git \
  curl \
  2>/dev/null || error "Falha ao instalar dependências"
...
PLATFORM="debian"
[[ "$DISTRO" == "ubuntu" ]] && PLATFORM="ubuntu"
[[ "$DISTRO" == "fedora" ]] && PLATFORM="fedora"
curl -sf -X POST "$TRACK_URL" \
  -H "Content-Type: application/json" \
  -d "{\"method\":\"apt\",\"platform\":\"$PLATFORM\",\"version\":\"3.0.0\",\"arch\":\"$ARCH_CPU\"}" \
  --max-time 5 || true

echo ""
success "CommitForge instalado com sucesso no $DISTRO!"
echo ""
